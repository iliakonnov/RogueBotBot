...
from Bot import BotState, ReplyResult
from Bot.ReplyUtils import go_away, reply, concat, dice, get_reply, conditional, set_item, battle, no_pet, nothing
...
def room_human(msg: str, replies: Sequence[str], state: BotState) -> ReplyResult:
    if 'Растеряться' in replies:  # Билл Гейтс
        ...
        return go_away(msg, replies, state)
    ...
    for var in [
        ...
        if var in replies:
            return var
    return concat(reply('Войти спиной вперёд'), )
...
rooms: Sequence[Room] = [
    ...
    Room("Билл Шифр",
         ...
         reply("Забирай, они мне не нужны")),
    Room("Василиск",
    ...
    Room(("Вода", "_Вода_"),
         ...
         concat(reply('Ты видишь сундук! Доплыть до него'), dice)),
    Room("Волк-оборотень",
    ...
    Room("Гидеон",
         ...
         concat(
             dice,
             get_reply(conditional(
                 lambda msg, _, __: (
                         'Что-о-о? Как ты это сделал?' in msg or 'От шока Гидеон выронил дневник из рук.' in msg
                 ),
                 set_item('Книга тайн №2', 1)
             )),
             reply("Ping"),  # Иначе основной цикл не получит сообщение =(
             ignore_func=None
         )),
    Room("Гном в красной шапке",
    ...
    Room("Дети",
         ...
         concat(reply('Гадость'), reply('➰ Использовать: ✨ Воображение'), dice)),
    Room("Джейсон Вурхиз",
    ...
    Room("Диппер",
         ...
         concat(
             battle,
             set_item("Книга тайн №3", 1),
             ignore_func=None
         )),
    Room("Длинноволосая брюнетка",
    ...
    Room("Кабинет директора",
         ...
         go_away),
    Room('Казино "Марианская Впадина"',
    ...
    Room("Какой-то игрок",
         ...
         go_away),
    Room("Какой-то мужчина",
         {'rooms/default/usual/witcher.py': 'c46e4f4898d122faf7239c50effc5cc5f6900fa1'},
         reply('Ты носишь двуручник за спиной? Ты в каком фильме это видел?')),
    Room("Караван Дварфов",
    ...
    Room("Катапульта",
         ...
         concat(reply("@iliago"), reply("Привет от бота!"))),
    Room("Книга судьбы",
    ...
    Room("Леприкон",
         ...
         battle),
    Room(('Лес', 'Дорога', 'Дупло', 'Ничего', 'Мама'),
    ...
    Room("Мишень",
         ...
         concat(reply('Попробовать попасть в нее'), dice)),
    Room("Мост",
    ...
    Room("Парикмахерская",
         ...
         reply('Подстричься за 10 золотых')),
    Room("Подсказка",
    ...
    Room("Сокровищница",
         ...
         concat(reply('NEVER GONNA GIVE YOU UP!'),
                reply('NEVER GONNA RUN AROUND!'),
                reply('NEVER GONNA MAKE YOU CRY!'))),
    Room("Спортзал",
    ...
    Room("Уютная комната",
         ...
         go_away),
    Room("Хиппи", {'rooms/vietnam/monster/hippie.py': 'b9f99e7b94fd0219b04e8204921058f26ce769bd'},
         battle),
    Room("Цезарь",
